...
public class ChipsLayoutManager extends RecyclerView.LayoutManager implements IChipsLayoutManagerContract,
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (state.isPreLayout()) {
            ...
            fill(recycler, layouterFactory.getBackwardLayouter(anchorView), layouterFactory.getForwardLayouter(anchorView));

            isAfterPreLayout = true;
        } else {
            detachAndScrapAttachedViews(recycler);
            if (anchorView.getPosition() > state.getItemCount()) {
                anchorView.setPosition(0);
            }
    ...
    private void fill(RecyclerView.Recycler recycler, ILayouter backwardLayouter, ILayouter forwardLayouter) {
        ...
        Log.d(TAG, "startingPos:" + startingPos);
        fillCache();
    ...
    public void onItemsMoved(RecyclerView recyclerView, int from, int to, int itemCount) {
        Log.d("onItemsMoved", String.format(Locale.US, "from = %d, to = %d, itemCount = %d", from, to, itemCount));
